...
# - Corrige bug de prompt de senha, montagem de diretórios, e inclui hardening.
#
set -euo pipefail
IFS=$'\n\t'
...
# Confere execução no Arch Live
[[ -e /etc/arch-release ]] || { echo "Este script só deve ser rodado em Arch Linux Live!"; exit 1; }
...
collect_passwords_interactive() {
  ...
  while true; do
    echo
    echo -ne "${BLUE}Senha LUKS (principal): ${NC}"
    read -rsp "" LUKS_PASS; echo
    echo -ne "${BLUE}Confirme LUKS: ${NC}"
    read -rsp "" tmp; echo
    [[ "$LUKS_PASS" == "$tmp" && -n "$LUKS_PASS" ]] || { warn "LUKS: senhas não coincidem ou vazia"; continue; }

    if [[ "$ENABLE_AUTO_DESTRUCTION" -eq 1 ]]; then
      echo -ne "${BLUE}Senha AUTO-DESTRUIÇÃO (mínimo 8 caracteres): ${NC}"
      read -rsp "" DESTRUCTION_PASS; echo
      echo -ne "${BLUE}Confirme destruição: ${NC}"
      read -rsp "" tmp; echo
      [[ "$DESTRUCTION_PASS" == "$tmp" && ${#DESTRUCTION_PASS} -ge 8 ]] || { warn "Destruição: senhas não coincidem ou muito curta"; continue; }
    fi

    echo -ne "${BLUE}Senha root: ${NC}"
    read -rsp "" ROOT_PASS; echo
    echo -ne "${BLUE}Confirme root: ${NC}"
    read -rsp "" tmp; echo
    [[ "$ROOT_PASS" == "$tmp" && -n "$ROOT_PASS" ]] || { warn "Root: senhas não coincidem ou vazia"; continue; }

    echo -ne "${BLUE}Senha do usuário $USERNAME: ${NC}"
    read -rsp "" USER_PASS; echo
    echo -ne "${BLUE}Confirme senha do usuário: ${NC}"
    read -rsp "" tmp; echo
    [[ "$USER_PASS" == "$tmp" && -n "$USER_PASS" ]] || { warn "Usuário: senhas não coincidem ou vazia"; continue; }

    echo -ne "${BLUE}PIN para desbloquear HD (dados): ${NC}"
    read -rsp "" PIN_DATA; echo
    echo -ne "${BLUE}Confirme PIN: ${NC}"
    read -rsp "" tmp; echo
...
check_required_tools() {
  ...
  if [[ ${#missing_tools[@]} -gt 0 ]]; then
    fatal "Ferramentas necessárias não encontradas: ${missing_tools[*]}. Instale antes de rodar."
  fi
...
mount_filesystems_for_install() {
  ...
  mkdir -p /mnt/boot /mnt/boot/efi /mnt/home /mnt/home/dados
...
install_base_and_prepare() {
  ...
  base_packages=(
    ...
    reflector nano sudo openssh networkmanager nftables audit aide
  )
  if timeout 5 ping -c1 8.8.8.8 >/dev/null 2>&1; then
...
sed -i "s|^GRUB_CMDLINE_LINUX=.*|GRUB_CMDLINE_LINUX=\"cryptdevice=UUID=${LUKS_UUID}:cryptroot root=/dev/vg_system/lv_root lockdown=confidentiality\"|" /etc/default/grub
