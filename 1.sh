#!/bin/bash
# ARCH SECURE SETUP - VERSÃO CORRIGIDA E COMPLETA
# Este script gera todos os arquivos necessários para instalação segura do Arch Linux
...
# ============================================================================
# FASE 1 - PREPARO (JÁ REVISADO - MANTIDO COMO ESTÁ)
# ============================================================================
...
validate_config_file() {
    if [[ "$NON_INTERACTIVE" == "true" ]]; then
        ...
        local required_fields=("disco_principal" "hostname" "username" "user_password" "root_password" "luks_root_password")
        for field in "${required_fields[@]}"; do
            local value=$(jq -r ".$field // \"\"" "$CONFIG_FILE")
            if [[ -z "$value" ]] || [[ "$value" == "null" ]]; then
                log "$RED" "Campo obrigatório ausente ou vazio: $field"
                exit 1
            fi
        done
    fi
